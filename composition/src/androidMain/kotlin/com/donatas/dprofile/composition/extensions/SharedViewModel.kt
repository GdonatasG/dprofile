...
): Pair<Module, InstanceFactory<T>> = viewModel<T>(qualifier = qualifier, definition = definition)

actual inline fun <reified T : ViewModel> ScopeDSL.sharedViewModel(
    qualifier: Qualifier?,
    noinline definition: Definition<T>
): KoinDefinition<T> = viewModel<T>(qualifier = qualifier, definition = definition)
